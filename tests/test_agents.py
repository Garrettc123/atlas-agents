"""
ATLAS Agent Unit Tests
Runs without API keys using mocks.
"""

import pytest
from unittest.mock import MagicMock, patch
from agents.outreach import is_allowed_send_time, is_opt_out_message, OutreachAgent
...
class TestSendTimeCompliance:
    ...
    def test_blocks_early_morning(self):
        with patch('agents.outreach.datetime') as mock_dt:
            mock_now = MagicMock()
            mock_now.hour = 7  # 7 AM — before 9 AM Texas window
    ...
    def test_blocks_late_night(self):
        with patch('agents.outreach.datetime') as mock_dt:
            mock_now = MagicMock()
            mock_now.hour = 22  # 10 PM — after 9 PM
            mock_dt.now.return_value = mock_now
            assert is_allowed_send_time() is False
...
class TestOptOutDetection:
    ...
    @pytest.mark.parametrize("msg", [
        'STOP', 'stop', 'Stop', 'QUIT', 'CANCEL',
        'UNSUBSCRIBE', 'OPT OUT', 'REMOVE'
    ])
    def test_detects_standard_opt_outs(self, msg):
        assert is_opt_out_message(msg) is True

    @pytest.mark.parametrize("msg", [
        'Yes please', 'Call me at 2pm', 'What is the price?',
        'I need a quote', 'OK'
    ])
